...
### 1. 대량 트랜잭션 정합성 + 외부 시스템 연동
수천 개 고객사가 사용하는 세금계산서 SaaS에서 월 120만 건 규모 발행 트랜잭션과 다양한 외부 시스템 연동을 실무로 담당. 신입 시절에는 PG 재정산 솔루션 소스 분석·유지보수를 담당하며 결제·정산 도메인 흐름을 접함
...
- **대량 트랜잭션 정합성이 중요한 서비스** (발행·정산·거래처럼 데이터 흐름의 정확도가 핵심인 시스템)
...
대규모 분산 시스템, Kubernetes, Kafka 기반 실서비스 운영 경험은 아직 부족합니다  
대신 실제 운영 서비스에서 장애를 끝까지 추적하고, 서명 모듈을 직접 개발해 라이브에 반영하며, 대량 트랜잭션과 다양한 외부 시스템 연동 흐름을 실무로 다뤄온 경험을 강점으로 가져가고 있습니다
